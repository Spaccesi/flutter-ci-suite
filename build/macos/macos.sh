...
if [ "$CODE_SIGN" != 'false' ]; then
  ...
  echo "::group::ℹ️ Patching project.pbxproj (UUID: $UUID)"
  ...
  echo "Current PROVISIONING_PROFILE_SPECIFIER lines after patch:"
  grep "PROVISIONING_PROFILE_SPECIFIER" "$PBXPROJ" || echo "(none found)"
  echo "::endgroup::"
